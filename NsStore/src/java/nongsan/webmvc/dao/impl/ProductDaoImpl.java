...
public class ProductDaoImpl extends connectDB implements ProductDao {
    ...
    public void edit(Product product) {
        if (product == null || product.getId() == null || !product.getId().matches("\\d+")) {
            throw new IllegalArgumentException("Invalid ID: must be a positive integer");
        }
        ...
        try (Connection con = connectDB.getConnect(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            throw new RuntimeException("Edit failed: " + e.getMessage());
        }
    }
    ...
    public List<Product> getProductById(int id) {
        if (id <= 0) {
            throw new IllegalArgumentException("catalog_id must be positive integer");
        }
        List<Product> products = new ArrayList<Product>();
