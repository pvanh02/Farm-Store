...
public class OrderedDaoImpl implements OrderedDao {
    ...
    public void insert(Ordered ordered) {
        if (ordered == null) {
                throw new NullPointerException("ordered cannot be null");
        }
        if (ordered.getQty() < 0) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
            ...
            throw new RuntimeException("Invalid format for product_id or transaction_id: " + e.getMessage(), e);
        } catch (SQLException e) {
            e.printStackTrace();
            throw new RuntimeException("Database error during insert: " + e.getMessage(), e);
        }
    }
